...
> **关联 commit**：`bbeb0a2`（plan Task 6-9 + models/__init__.py 引入 3 model）
> **目的**：面试前复习 + ORM 设计原则
...
**为什么用字符串引用**：
- 避免循环 import（A 引 B，B 引 A → ImportError）
- 不强制顺序：User.py 和 UserGoal.py 谁先 import 都行
- 代价：失去 IDE 跳转（点 "UserGoal" 不会跳转）
...
## 5. ENUM 在 DB 层 vs Python 校验（双层防御）
...
> **面试话术**：「BodyMeasurement 用 `recorded_at`（业务时间）+ `created_at`（系统时间）两个字段——业务时间可补录历史，系统时间不可改。两个时间分离处理'补录 7 天前的测量'这种场景。我从来不用单一时间字段——上线后排查 bug 时经常需要'用户视角的时间'和'系统视角的时间'分开查。」
...
| 循环 import | ImportError on startup | 用字符串引用 + 把所有 model 在 models/__init__.py 集中 import |
